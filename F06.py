...
def ubah_stok(dfgame):
    ...
    if found == True:  # Jika terdapat ID game dalam game.csv
        ...
        newStok = int(input("Masukkan jumlah: "))
        if (newStok > 0):
            stok += newStok
            dfgame[i][5] = stok
            print("Stok game " + dfgame[i][1] + " berhasil ditambahkan. Stok sekarang: " + str(stok))

        elif (
                newStok < 0):  # Jika ingin mengurangi stok harus dilihat apakah jumlah yang ingin dikurang menyebabkan stok sekarang menjadi negatif atau tidak
            if stok < (newStok * (-1)):
